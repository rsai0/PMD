...
def load_data(stage:str='train', **kwargs):
    ...
    if kwargs["RS_Vars"] is None:
        RS_Vars = ["albedo_01", "albedo_02", "albedo_03"
    ...
    if kwargs["Cld_Vars"] is None:
        Cld_Vars = ['CLTYPE', 'CLOT', 'CLTT', 'CLTH', 'CLER_23']   #['CLTYPE', 'CLOT', 'CLTT', 'CLTH', 'CLER_23']
    else:
        Cld_Vars = kwargs["Cld_Vars"]
    Combine_var = list(itertools.combinations(range(1, 18), 1))+list(itertools.combinations(range(1, 7), 2))+list(itertools.combinations(range(7, 17), 2))
    ...
    if kwargs["RS_file"] is not None:
        ...
        if kwargs["RS_Threshold"] is not None:
            ...
            RS_temp1 = np.stack([data.variables[RS_Var].values for RS_Var in RS_Vars], axis=0)
        RS_Data = []
        for ii in Bidx:
            try:
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, params, mask_ratio:float=-1.0, mask_bands=None, stage:str='train', hw=[5, 5]):
        """
        ...
        if params["is_norm"]:
            import pandas as pd
            vr = pd.read_csv(params["vr_file"], index_col=0)
        if "add_vars" in params and params["add_vars"] is not None:
            add_data = []
            for xname in params["add_vars"]:
                if xname == "latitude":
                    add_temp0 = np.loadtxt(params["latitude_file"])
                    if params["is_norm"]:
                        xmin = vr[xname]["min"]
                        xmax = vr[xname]["max"]
                        add_temp0 = (add_temp0-xmin)/(xmax-xmin)
                elif xname == "longitude":
                    add_temp0 = np.loadtxt(params["longitude_file"])
                    if params["is_norm"]:
                        xmin = vr[xname]["min"]
                        xmax = vr[xname]["max"]
                        add_temp0 = (add_temp0-xmin)/(xmax-xmin)
                elif xname == "attitude":
                    add_temp0 = np.loadtxt(params["earth_relief_file"])
                    ...
                    add_temp0 = np.loadtxt(params["earth_mask_file"])
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_ratio>0.0 and self.mask_ratio<=1.0:
            mask_id = np.random.choice([True, False], out['RS'].shape[-2:], p=[self.mask_ratio, 1.0-self.mask_ratio])
            for mask_band in self.mask_bands:
                out['RS'][mask_band, mask_id] = 0

        if self.stage != 'inference':
            if self.geoinfo is None:
                return [out["RS"][:, :-1, :-1]] + [out[xx][:-1, :-1] for xx in self.params['Cld_Vars']]
            else:
                return [np.concatenate([out["RS"], self.geoinfo], axis=0)[:, :-1, :-1]] + [out[xx][:-1, :-1] for xx in self.params['Cld_Vars']]
        else:
            if self.geoinfo is None:
                return out["RS"][:, :-1, :-1]
            else:
                return np.concatenate([out["RS"], self.geoinfo], axis=0)[:, :-1, :-1]
